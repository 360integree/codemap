...
class JsTsExtractor(BaseExtractor):
    ...
    RE_TYPE = re.compile(r"""(?:export\s+)?type\s+(\w+)""")
    RE_FUNCTION = re.compile(r"""(?:export\s+)?(?:async\s+)?function\s+(\w+)""")
    RE_CONST_FUNC = re.compile(r"""(?:export\s+)?const\s+(\w+)\s*=\s*(?:async\s+)?\(""")
    RE_ROUTE = re.compile(
        r"""(?:app|router)\.(?:get|post|put|delete|patch|route)\s*\(['"]([^'"]+)['"]"""
    )
    RE_REACT_COMPONENT = re.compile(r"""(?:export\s+)?(?:default\s+)?function\s+([A-Z]\w+)""")
