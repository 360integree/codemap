...
from projetmap.extractors.base import BaseExtractor, Entity, ExtractionResult


class GenericExtractor(BaseExtractor):
    """Generic regex-based extractor for any source file."""

    RE_IMPORT = re.compile(r"""(?:import|include|require|use)\s+['"<]([^'">]+)['">]""")
    RE_CLASS = re.compile(r"""(?:class|struct|interface|trait|type|record)\s+(\w+)""")
    RE_FUNCTION = re.compile(r"""(?:function|def|fn|func|sub|procedure|method)\s+(\w+)""")
