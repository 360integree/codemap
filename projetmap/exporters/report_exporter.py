...
def export_report(graph_data: dict, output_path: Path) -> Path:
    ...
    if god_nodes:
        ...
        for i, gn in enumerate(god_nodes[:10], 1):
            ...
            lines.append(f"### {i}. `{node_id}` — {connections} connections ⭐")
    ...
    meaningful_clusters = [c for c in clusters if len(c.get("members", [])) > 1]
    if meaningful_clusters:
        ...
        for i, cluster in enumerate(meaningful_clusters[:25], 1):
            ...
            lines.append(f"| {i} | {cluster['name']} | {member_count} | {preview[:70]} |")
        lines.append("")
