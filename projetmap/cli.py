...
Usage:
    ...
    python -m projetmap <path> --journeys         # User journey detection
...
def run_instruction_analysis(
    ...
    print("   📄 instruction_analysis.json")
...
def run_journey_analysis(
    ...
    """Run user journey detection and export."""
    ...
    print(f"🗺️  Codemap User Journey Analysis — Scanning {root}")
    ...
    # JSON
    with open(out / "user_journeys.json", "w") as f:
        json.dump(report.to_dict(), f, indent=2, default=str)

    # HTML
    export_journeys_html(report, out / "user_journeys.html")

    # Markdown report
    export_journeys_report(report, out / "USER_JOURNEYS.md")

    # Append to main report
    _append_journey_report(report, out / "GRAPH_REPORT.md")

    summary = report.to_dict()["summary"]
    print(f"\n✅ User journey analysis saved to {out}/")
...
def _append_journey_report(report, report_path: Path):
    """Append user journey section to the main report."""
    lines = [
        "",
        "---",
        "",
    ...
    # Don't duplicate
    if "## User Journeys" in existing:
        idx = existing.find("## User Journeys")
        if idx > 0:
            existing = existing[:idx]
    ...
    with open(report_path, "w") as f:
        f.write(existing)
        f.write("\n".join(lines))
        f.write("\n\n---\n*Generated by Codemap — User Journey Analysis*\n")


def install_skill():
    """Install ZCode skill to ~/.agents/skills/projetmap/."""
    import shutil
    from importlib import resources

    skill_dir = Path.home() / ".agents" / "skills" / "projetmap"
    skill_source = Path(str(resources.files("projetmap") / "skill"))

    if not skill_source.exists():
        print("❌ Skill files not found in package. Reinstall with: pip install -e .")
        return

    if skill_dir.exists():
        print(f"⚠️  Skill already exists at {skill_dir}")
    ...
    print(f"📥 Installing skill to {skill_dir}...")
    ...
    print("")
    print("Usage in ZCode:")
    print("  /projetmap <path>           # Scan project")
    print("  /projetmap <path> --journeys # Scan with user journeys")


def uninstall_skill():
    """Uninstall ZCode skill from ~/.agents/skills/projetmap/."""
    import shutil
    ...
    print(f"⚠️  This will remove: {skill_dir}")
    ...
    shutil.rmtree(skill_dir)
    print(f"✅ Skill uninstalled from {skill_dir}")
...
def uninstall():
    ...
    # 2. Remove MCP configs
    print("")
    for ide, path in MCP_CONFIG_PATHS.items():
        if not path.exists():
            continue
        try:
            with open(path) as f:
                config = json.load(f)
            if "mcpServers" in config and "projetmap" in config["mcpServers"]:
                del config["mcpServers"]["projetmap"]
                if not config["mcpServers"]:
                    del config["mcpServers"]
                with open(path, "w") as f:
                    json.dump(config, f, indent=2)
                print(f"🔌 MCP ({ide}): {path}")
                print("   ✅ Removed")
    ...
    # 3. Pip uninstall
    print("")
    print("📦 Package: pip uninstall projetmap")
    response = input("   Run pip uninstall now? (y/n) ").strip().lower()
    if response == "y":
        result = subprocess.run([sys.executable, "-m", "pip", "uninstall", "projetmap", "-y"])
        if result.returncode == 0:
            print("   ✅ Package uninstalled")
        else:
            print("   ⚠️  pip uninstall failed — run manually: pip uninstall projetmap")
    else:
        print("   ⏭️  Skipped — run manually: pip uninstall projetmap")

    print("")
    print("✅ Projetmap uninstalled")
...
def generate_mcp_config(python_path: str, project_path: str) -> dict:
...
def install_mcp(ide: str):
    """Install MCP config for specified IDE."""
    import json
    ...
    if ide == "auto":
        ide = detect_ide()
        if not ide:
            print("❌ No supported IDE detected. Specify one: cursor, claude, windsurf, zcode")
            return
        print(f"🔍 Detected IDE: {ide}")

    if ide not in MCP_CONFIG_PATHS:
        print(f"❌ Unknown IDE: {ide}")
        print(f"   Supported: {', '.join(MCP_CONFIG_PATHS.keys())}")
        return

    config_path = MCP_CONFIG_PATHS[ide]
    ...
    if config_path.exists():
        try:
            ...
            print(f"⚠️  Invalid JSON in {config_path}, will overwrite")

    # Add MCP server config
    if "mcpServers" not in config:
        config["mcpServers"] = {}

    if "projetmap" in config["mcpServers"]:
        print(f"⚠️  projetmap MCP config already exists in {config_path}")
        response = input("Update? (y/n) ").strip().lower()
        if response != "y":
            print("Cancelled.")
            return

    config["mcpServers"]["projetmap"] = generate_mcp_config(python_path, project_path)
    ...
    print(f"✅ MCP config installed for {ide}")
    print(f"   Config: {config_path}")


def uninstall_mcp(ide: str):
    """Uninstall MCP config from specified IDE."""
    import json
    ...
    if "mcpServers" not in config or "projetmap" not in config["mcpServers"]:
        print(f"❌ projetmap MCP config not found in {config_path}")
        return

    print(f"⚠️  This will remove projetmap MCP config from: {config_path}")
    response = input("Continue? (y/n) ").strip().lower()
    if response != "y":
        print("Cancelled.")
        return
    ...
    print(f"✅ MCP config uninstalled from {ide}")
...
def main():
    ...
    if args.uninstall_mcp is not None:
        uninstall_mcp(args.uninstall_mcp)
        return
    ...
    # Run user journey detection if requested
    if args.journeys:
